...
static enum ifpga_sec_err check_and_do_cancel(struct ifpga_sec_mgr *imgr)
{
	...
	if (imgr->progress == IFPGA_SEC_PROG_PREPARING ||
	    imgr->progress == IFPGA_SEC_PROG_WRITING)
		imgr->iops->cancel(imgr);
	return true;
}

static bool progress_transition(struct ifpga_sec_mgr *imgr,
				enum ifpga_sec_prog new_progress)
{
	mutex_lock(&imgr->lock);
	if (!check_and_do_cancel(imgr))
		update_progress(imgr, new_progress);
	mutex_unlock(&imgr->lock);
	return imgr->err_code ? false : true;
}

static void ifpga_sec_error(struct ifpga_sec_mgr *imgr,
...
static void ifpga_sec_mgr_update(struct work_struct *work)
{
	...
	if (request_firmware(&fw, imgr->filename, &imgr->dev)) {
		ifpga_sec_error(imgr, IFPGA_SEC_ERR_FILE_READ);
		goto idle_exit;
	}
	...
	if (!try_module_get(imgr->dev.parent->driver->owner)) {
		ifpga_sec_error(imgr, IFPGA_SEC_ERR_BUSY);
 ...
 *
 * NOTE: The caller of this function must subsequently call
 * ifpga_sec_mgr_register(imgr) to register the security manager
 * device, or release the device using put_device(&imgr->dev).
 *
 ...
 *
 * NOTE: if device registration fails, the device will be
 * released, so no cleanup is required by the caller.
 *
